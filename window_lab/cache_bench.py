...
def run(out:Path,*,live_jev=False,live_llm=False):
    ...
    for strategy in STRATEGIES:
        ...
        summary["strategies"][strategy]={
          ...
          "known_llm_cost_usd":sum(c.get("cost_usd") or 0 for c in calls),
    ...
    lines=["# Window/cache economics smoke","",
      "|strategy|correct/12|input tokens|cached tokens|uncached tokens|LLM list-cost USD|mean byte-prefix reuse|",
      "|---|---:|---:|---:|---:|---:|---:|"]
    for k,v in summary["strategies"].items():
        lines.append(f"|{k}|{v['correct'] if v['correct'] is not None else 'unknown'}/12|{v['input_tokens']}|{v['cached_input_tokens']}|{v['uncached_input_tokens']}|{v['known_llm_cost_usd']:.6f}|{v['mean_prefix_reuse_ratio']:.3f}|")
